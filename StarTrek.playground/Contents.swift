...
// A:  28.57%
let femHumanOfficers = humanOfficers.filter{femaleOfficers.contains($0)}
var femPercent: Float = (Float(femHumanOfficers.count) / Float(humanOfficers.count)) * 100
print(String(femPercent) + "%")


// Q2: How many first officers are human?
// A:  2
let compactFO = firstOfficers.compactMap{$0.key}
let humanFO = compactFO.filter{humanOfficers.contains($0)}
print(humanFO.count)

// Q3: Get a list of females sorted by name
// A: ["dax", "janeway", "kira", "seven", "tpol", "troi"]
...
let vulcanFO: [Officer] = firstOfficers.map{Officer(name: $0, ship: $1)}.filter{vulcanOfficers.contains($0.name)}
print(vulcanFO)



// Q7: How many these Vulcan first officers have names shorter than 5 characters?
print(vulcanFO.filter{$0.name.count < 5}.count)
